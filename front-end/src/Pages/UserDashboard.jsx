...
import PropertyCard from "../components/PropertyCard"; // A component to display property details
import { fetchUserMessages, fetchReplies, replyToMessage } from "../services/messageService";
        ...
        {messages.length > 0 ? (
          messages.map((msg) => (
            ...
            >
              ...
              <h2 className="text-lg font-semibold mt-4">Replies</h2>
              {msg.replies.length > 0 ? (
                msg.replies.map((reply) => (
                  <div key={reply._id} className="border p-3 my-2 rounded-lg bg-gray-50">
                    <p><strong>{reply.senderId?.name || "Owner"}</strong> ({reply.senderId?.email}):</p>
                    <p className="text-gray-700">{reply.message}</p>
                    <p className="text-sm text-gray-500">Sent: {new Date(reply.timestamp).toLocaleString()}</p>
                  </div>
                ))
              ) : (
              ...
              {/* Reply Input - reply to the original message thread */}
              <div className="mt-2 flex">
                <input
                  type="text"
                  placeholder="Type your reply..."
